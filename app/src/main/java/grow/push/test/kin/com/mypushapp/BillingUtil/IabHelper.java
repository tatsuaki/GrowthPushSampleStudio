        ...
        try {
            ...
            if (response == 0) {
                ...
                if (!ownedItems.containsKey(RESPONSE_INAPP_ITEM_LIST)
                    ...
                    Log.d(mDebugTag, "Bundle returned from getPurchases() doesn't contain required fields.");
                }
                int sizes = purchaseDataList.size();
                Log.d(mDebugTag, "sizes = " + sizes);
                Purchase purchase = null;
                for (int i = 0; i < purchaseDataList.size(); ++i) {
                    String purchaseData = purchaseDataList.get(i);
                    String signature = signatureList.get(i);
                    String sku = ownedSkus.get(i);
                    purchase = new Purchase(ITEM_TYPE_INAPP, purchaseData, signature);
                    mService.consumePurchase(3, mContext.getPackageName(), purchase.getToken());
                }
            }
        } catch (JSONException e) {
            e.printStackTrace();
